...
float* MPU6886::getAhrsData(float *pitch,float *roll,float *yaw, float aOX,float aOY,float aOZ,float gOX,float gOY,float gOZ){
  ...
  float* quatanion = MahonyAHRSupdateIMU((gyroX-gOX) * DEG_TO_RAD, (gyroY-gOY) * DEG_TO_RAD, (gyroZ-gOZ) * DEG_TO_RAD, (accX-aOX), (accY-aOY), (accZ-aOZ),pitch,roll,yaw);
  return quatanion;
}
